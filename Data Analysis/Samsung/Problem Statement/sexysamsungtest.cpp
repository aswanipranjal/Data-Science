...
void make_placeholder() {
	...
	for (size_t i = 2; i < filenames.size(); ++i) {
		...
		if(filenames[i] == string("places.txt")) v.push_back("london");
		stream.close();
...
int main() {
	...
	string input_str = "Can you tell me when will it rain in Bangalore after 6 a.m. tomorrow and Find out What is time in London now thanks ";
	...
	// 	cout << "\n***PlaceHolders***\n";
	// 	print_set(it->second.second.first);
	// 	cout << "\n***Keywords***\n";
	// 	print_set(it->second.second.second);
	// 	cout << "\n";
	// }

	// print_vec(placeholder["<places>"]);
