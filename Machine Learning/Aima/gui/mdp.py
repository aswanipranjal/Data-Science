...
def dialogbox(i, j, gridmdp):
	...
	label_reward = ttk.Label(container, text='Reward', font=('Helvetica', 10), anchor=tk.CENTER)
	...
	rb.grid(row=2, column=0, columnspan=3, sticky='nsew', padx=156, pady=5)
	btn_apply = ttk.Button(container, text='Apply', command=update_table)
	btn_apply.grid(row=3, column=0, sticky='nsew', pady=5, padx=5)
	btn_reset = ttk.Button(container, text='Reset', command=partial(reset_radio_button, rb))
	btn_reset.grid(row=3, column=1, sticky='nsew', pady=5, padx=5)
	btn_ok = ttk.Button(container, text='Ok', command=dialog.destroy)
	btn_ok.grid(row=3, column=2, sticky='nsew', pady=5, padx=5)
